...
# =========================
def print_retrieved_docs(retrieved_docs, max_content_len: int = 1000):
    print("\n" + "=" * 80)
    print("【检索到并送入 Prompt Builder 的文档】")
    print("=" * 80)
    ...
    for i, doc in enumerate(retrieved_docs, 1):
        ...
        print(f"score   : {getattr(doc, 'score', None)}")
        print(f"meta    : {getattr(doc, 'meta', None)}")
...
def print_llm_input(messages):
    print("\n" + "=" * 80)
    print("【最终送入大模型的 messages / prompt】")
    print("=" * 80)

    if messages is None:
        print("prompt_builder 没有返回 messages。")
        return

    # 有些实现返回 list[ChatMessage]，有些可能返回字符串，做兼容
...
def main():
    ...
    try:
        response = rag.run(
            ...
            include_outputs_from={"retriever", "prompt_builder", "llm"}   # 关键：强制保留中间输出
        )
        ...
        # =========================
        # 打印 prompt_builder 输出
        # =========================
        # 常见情况：prompt_builder 输出字段名为 "prompt"
        llm_input = response.get("prompt_builder", {}).get("prompt", None)
        print_llm_input(llm_input)
